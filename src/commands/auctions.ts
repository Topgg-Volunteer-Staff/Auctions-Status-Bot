...
export const execute = async (
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            {
                name: "🕓 Bidding Ends",
                value: `<t:${Math.floor(monday19UTC.getTime() / 1000)}:f> (<t:${Math.floor(monday19UTC.getTime() / 1000)}:R>)`
            },
            {
                name: "💳 Payment Due & New Bidding Opens",
                value: `<t:${Math.floor(tuesday19UTC.getTime() / 1000)}:f> (<t:${Math.floor(tuesday19UTC.getTime() / 1000)}:R>)`
            },
            {
                name: "📢 Ads Go Live",
                value: `<t:${Math.floor(tuesday20UTC.getTime() / 1000)}:f> (<t:${Math.floor(tuesday20UTC.getTime() / 1000)}:R>)`
            }
        )
        .setColor('#ff3366');
